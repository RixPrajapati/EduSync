...
import { mailRex, passwordRex } from "../../constants/regex.js";
...
const userSchema = z.object({
  ...
  email: z
    ...
    .trim()
    .regex(mailRex, { message: "Email is in not correct format " }),
  password: z
    .string({ required_err: "Password is required" })
    .trim()
    .min(8, { message: "Password is not  than 8 character" })
    .max(20, { message: "Password is not grater than 20 character" })
  ...
  phone: z
    .string({ required_err: "Phone is required" })
    .trim()
    .check(minLength(7), maxLength(15)),
  ...
  gender: z.enum([MALE, FEMALE, OTHER], { required_err: "Gender is required" }),
